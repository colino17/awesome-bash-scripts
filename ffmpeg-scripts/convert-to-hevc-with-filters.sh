...
for i in *.mp4;
  ...
  audioformat=$(ffprobe -loglevel error -select_streams a:0 -show_entries stream=codec_name -of default=nw=1:nk=1 "$i")
    if [ "$audioformat" = "aac" ]; then
      ffmpeg -i "$i" -vf "eq=contrast=1.2:saturation=1.5,unsharp,hqdn3d,yadif" -c:v libx265 -crf 23 -preset fast -c:a copy -movflags faststart "out/${i%.*}.mp4"
    else
      ffmpeg -i "$i" -vf "eq=contrast=1.2:saturation=1.5,unsharp,hqdn3d,yadif" -c:v libx265 -crf 23 -preset fast -c:a aac -movflags faststart "out/${i%.*}.mp4"
    fi
done
